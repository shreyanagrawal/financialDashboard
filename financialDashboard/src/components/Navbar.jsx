...
const Navbar = ({ open, ready, handleLogout, username, loading, sidebarOpen, setSidebarOpen }) => {
  const {isDataAvailable} = useContext(PlaidContext);
  return (
    <div className="sticky top-0 w-full bg-gradient-to-r from-blue-600 to-indigo-700 shadow-lg px-4 md:px-8 py-4 flex flex-wrap justify-between items-center gap-4 rounded-b-2xl" style={{"zIndex":99}}>
      <div className="flex items-center gap-4">
        ...
        >
           ☰
        </button><div className="flex items-center gap-2 pl-6 lg:pl-0">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 w-full md:w-auto">
        ...
        <button onClick={handleLogout} className="bg-red-500 text-white px-5 py-2 rounded-xl font-semibold hover:bg-red-600 hover:scale-105 transition duration-200 shadow-md cursor-pointer">Logout
        </button>
      </div>
    </div>
  );
